...
- **The Rust backend has now actually been compiled**, not just hand-reviewed.
  I got a Rust toolchain working and ran `cargo check` and a full `cargo
  build` against `main.rs` and all its dependencies (tray icon, window
  management, the background timer thread, every `#[tauri::command]`) -
  zero errors, zero warnings. I also syntax-checked all the JS, unit-tested
  the state/leveling/streak logic (including month/year boundary cases),
  and cross-checked every DOM id referenced from JS against the HTML files
  to catch typos, plus verified the permission sets each plugin needs
  against `capabilities/default.json`.
  This was a Linux build, so it can't produce the Windows `.exe` itself or
  fully exercise Windows-only paths (tray behavior, NSIS packaging) - that
  final step still happens via the GitHub Action or a Windows machine with
  Rust installed, as below. But the code itself - the part that previously
  had a real chance of hiding a silent typo or API mismatch - is now
  confirmed sound rather than just carefully read. If `npm run build` (or
  the GitHub Action) still throws something on Windows, paste it back and
  I'll fix it.
