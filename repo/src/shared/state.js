...
export async function loadState() {
  ...
  const before = { lastActiveDate: merged.lastActiveDate, todayDate: merged.stats.todayDate };
  const rolled = rolloverDailyFields(merged);
  // Persist immediately if the rollover changed anything (new day, streak bump,
  // daily counters reset). Without this, just opening the app on a given day
  // without also petting/focusing/editing a to-do never gets written to disk,
  // so the streak can silently fail to advance or even reset on the next launch.
  if (rolled.lastActiveDate !== before.lastActiveDate || rolled.stats.todayDate !== before.todayDate) {
    await store.set('state', rolled);
    await store.save();
  }
  return rolled;
}
